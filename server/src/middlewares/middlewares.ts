...
export const validateNumberParam = (paramName: string): RequestHandler => {
  ...
  const validator: RequestHandler = (req, res, next) => {
    try {
      ...
      const errorResponse = () =>
        res.status(400).json({ error: "Invalid number param" });
...
export const validateBooleanQueryParam = (
  paramName: string,
): RequestHandler => {
  console.log(
    "calling validateBooleanQueryParamWithParamWithParamName",
    paramName,
  );
  const validator: RequestHandler = (req, res, next) => {
    try {
      ...
      const errorResponse = () =>
        res.status(400).json({ error: "Invalid boolean query param" });
